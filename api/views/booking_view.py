...
class BookingViewSet(viewsets.ModelViewSet):
    ...
    def cancel_booking(self, request, pk=None):
        ...
        if timezone.now() > booking.created_at + timezone.timedelta(hours=24):
            return Response({
    ...
    def bookings_by_status(self, request):
        ...
        if not status_param:
            return Response({"error": "Missing 'status' query parameter."},
    ...
    def bookings_by_period(self, request):
        ...
        if not period:
            return Response({"error": "Missing 'period' query parameter (week or month)."},
                            status=status.HTTP_400_BAD_REQUEST)
        ...
        if period == 'week':
            ...
            return Response({"error": "Invalid period. Use 'week' or 'month'."},
                            status=status.HTTP_400_BAD_REQUEST)
